...
## Pro v2026.6.9
- 🔓 `JwtHelper.VerifyToken(string token)` 改为 **public**
   企业级客户可直接调用验证方法，按需组合或自定义认证中间件，无需重写算法。
...
➕ `AddStaticFiles` 静态文件中间件新增多级目录自动匹配 `index.html` 功能，路径以 `/` 结尾时自动返回对应目录下 `index.html`；示例：访问 `/`、`/admin/`、`/docs/v1/` 分别匹配根目录、admin、docs/v1 目录下的 index.html。让静态文件托管有更好的体验。
