...
## Pro v2026.5.28
➕ 新增 AddBasicAuth 基本身份认证中间件。可为指定路由（路径）快速增加认证保护，无需编写登录页面，浏览器侧会自动弹出登录对话框。特别适合 Web 配置页面。

## V1.7.3
- ➕ `websocket` 服务端新增WsCloseClientAsync和WsCloseAllClientsAsync方法，用于服务端主动断开客户端。
